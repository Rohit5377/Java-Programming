...
class MinimizedMaximumOfProductsDistributedToStore {
    ...
    public boolean isPossible(int[] quantities, int n, int size){
        int count=0;
        for(int i=0;i<quantities.length;i++){
            count+=quantities[i]/size;
            if(quantities[i]%size!=0){
                count++;
            }
            if(count>n){
                return false;
